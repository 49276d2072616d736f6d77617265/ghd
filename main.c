...
int main(void) {
    ...
    for (;;) {
        ...
        int received = http_read_request(client, buf, sizeof(buf), &header_len, &content_length);
        if (received < 0 || content_length <= 0 || content_length > MAX_BODY) {
            http_send_unauthorized(client); close(client); continue;
        }
        ...
        if (http_read_body(client, body, content_length, already) < 0) {
            free(body); close(client); continue;
        }
        ...
        if (parse_webhook_json(body, content_length, &ev) == 0) {
            ...
            ghp_status_t status = github_should_process(&ev);

            switch (status) {
                case GHP_OK:
                    if (exec_event(&ev) == 0)
                        printf("[ghd] pull executed successfully\n");
                    else
                        printf("[ghd] pull execution failed\n");
                    break;

                case GHP_IGNORE_ACTION:
                    printf("[ghd] ignored: action=%s\n", ev.action); break;
                case GHP_IGNORE_REPO:
                    printf("[ghd] ignored: repo=%s\n", ev.repo); break;
                case GHP_IGNORE_BRANCH:
                    printf("[ghd] ignored: branch=%s\n", ev.ref); break;
                default:
                    printf("[ghd] ignored: status=%d\n", status); break;
            }
        } else {
